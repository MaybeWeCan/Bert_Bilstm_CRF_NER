...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_gpu(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            # pred_ids, label,
            # seq_length: （1）mask计算长度， （2）但是我记得有一个最长长度，可以看
            # word_to_id, label_to_id

            for pred_seq, gold_seq, input_seq in zip(pred_ids,
                                                        label_ids,
                                                        input_ids):

                pred_seq = [i2w_bio[i] for i in pred_seq]
                gold_seq = [i2w_bio[i] for i in gold_seq]
                char_seq = [i2w_char[i] for i in input_seq]
            ...
            if (f1 - best_f1) >= 0.01:
                best_f1 = f1
                best_step = tf.train.get_or_create_global_step()
                logger.info("Now Best F1 is : {} step is {}".format(best_f1,best_step))
...
if __name__ == "__main__":
    ...
    i2w_char = load_vocabulary(arg_dic["vocab_file"])
    i2w_bio = load_vocabulary(arg_dic["data_dir"]+"label.txt")
    ...
    logger.info(
        "Final Best F1 is : {} step is {}".format(best_f1, best_step))